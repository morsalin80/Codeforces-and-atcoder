...
void solution(){
    ...
    f0(n){cin>>x[i]>>y[i];xx.pb(mp(x[i],i));yy.pb(mp(y[i],i));}
    sort(ALL(xx));
    sort(ALL(yy));
   	int xl=n-1,yl=n-1;
   	int xp=xx[n-1].fi-xx[0].fi;
   	int yp=yy[n-1].fi-yy[0].fi;
   //	debug(xp,yp);
   	vi vec;
   	if(xp>yp){
   		vec.pb(xx[n-1].fi-xx[1].fi);
   		xl--;
   		if(xx[n-1].se==yy[n-1].se)yl--;
   	}
   	else{
   		vec.pb(yy[n-1].fi-yy[1].fi);
   		yl--;
   		if(xx[n-1].se==yy[n-1].se)xl--;
   	}
   	//debug(xl,yl);
   	//vi vec;
   	vec.pb(xx[xl].fi-xx[0].fi);
   	vec.pb(xx[xl].fi-xx[1].fi);
   	vec.pb(yy[yl].fi-yy[0].fi);
   	vec.pb(yy[yl].fi-yy[1].fi);
   	sort(ALL(vec));
   	reverse(ALL(vec));
   	cout << vec[0] << endl;
}
