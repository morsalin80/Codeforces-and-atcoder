...
void solution(){
    int n,x;
    cin>>n>>x;
    int arr[n];f0(n)cin>>arr[i];
    ///for 0 : all the contestants already infected
    bool zero=1;
    f0(n){
    	if(arr[i]!=x){
    		zero=0;
    ...
    ///for 1: there has to be a infected element or the sum of diff between arr[i] and x has to be zero
    bool one=0;
    f0(n){
    	if(arr[i]==x){
    		one=1;
