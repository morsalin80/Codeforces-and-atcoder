...
void solution(){
   ...
   // cout << p[0].fi << " "<< p[0].se << endl << p[1].fi << " "<< p[1].se << endl;
    if(p[0].fi==p[1].fi){
    	if(p[0].fi==n-1)mat[p[0].fi-1][p[0].se]='*';
    	else{
    		mat[p[0].fi+1][p[0].se]='*';
    	}
    	if(p[1].fi==n-1)mat[p[1].fi-1][p[1].se]='*';
    	else{
    		mat[p[1].fi+1][p[1].se]='*';
    	}
    }
    else if(p[0].se==p[1].se){
    	if(p[0].se==n-1)mat[p[0].fi][p[0].se-1]='*';
    	else{
    		mat[p[0].fi][p[0].se+1]='*';
    	}
    	if(p[1].se==n-1)mat[p[1].fi][p[1].se-1]='*';
    	else{
    		mat[p[1].fi][p[1].se +1]='*';
    	}
    }
