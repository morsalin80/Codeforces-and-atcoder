...
///solution: This solution is inspired from :: theRabiulAwal ::
vector<int>vec(1005);
string s;
int m;
bool dfs(int now,int lvl){
    if(lvl==m+1){ ///all m weights are well placed earlier.
        cout << "YES\n";
        for(int i=1;i<=m;i++){
            cout << vec[i] << " ";
        }
    ...
    for(int i=now+1;i<=10;i++){
        if(s[i-1]=='1' && i!=vec[lvl-1]){
            vec[lvl]=i;
            if(dfs(i-now,lvl+1)) return 1;///checking is there any option available for next move.
        }
    }
    return 0;
}
