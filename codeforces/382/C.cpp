...
void solution(){
    ...
    if(diff.size()==1){
        auto it=diff.begin();
        if(n==2 && (arr[0]+arr[1])%2==0 && it->fi!=0){
           cout << 3 << endl;
           cout << arr[0]-it->fi << " "<< (arr[0]+arr[1])/2 << " "<< arr[1]+it->fi << endl;
        }
        else if(it->fi!=0){
            cout << 2 << endl;
            //auto it=diff.begin();
            cout << arr[0]-it->fi << " "<< arr[n-1]+it->fi << endl;
        }
        else{
            cout << 1 << endl;
            cout << arr[0] << endl;
    ...
    auto it=diff.begin();
    int xx=it->se,x=it->fi;
    it++;
    int yy=it->se,y=it->fi;
    if(xx==1 && x==y*2){
        cout << 1 << endl;
        for(int i=1;i<n;i++){
            if(arr[i]-arr[i-1]==x){
