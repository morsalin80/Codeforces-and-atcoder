        ...
        for(int j=0;j<m;j++){
            if(s[i][j]=='*'){
               ...
               // debug(i,j,ans);
                ...
                int cnt=0;
                for(int k=j-1;k>=0 && k>=j-mark[i][j];k--){
                    if(s[i][k]!='*'){
                        break;
                    }
                    left++;
                }
                for(int k=j+1;k<m && k<=j+mark[i][j];k++){
                    if(s[i][k]!='*'){
                        break;
                    }
                    right++;
                }
                if(left==mark[i][j] && right==mark[i][j]){
                    ans+=mark[i][j];
                    mark[i+1][j]=mark[i][j]+1;
                }
                else {mark[i+1][j]=min(left,right)+1;
                    ans+=min(left,right);
                }
               //debug(left,right,cnt,ans);
                }
