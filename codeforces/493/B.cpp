...
void solution(){
    ...
    bool last;
    ll nsum=0,psum=0;
    f0(n){cin>>arr[i];if(arr[i]>0){pos.pb(arr[i]);last=1;psum+=arr[i];}else {neg.pb((-1)*arr[i]);last=0;nsum+=(-1*arr[i]);}}
    if(nsum>psum){
        cout << "second\n";
    }
    else if(nsum<psum){
        ...
        bool got=0;
        for(int i=0;i<(int)neg.size() && i<(int)pos.size();i++){
            if(neg[i]>pos[i]){
                got=1;
                cout << "second\n";
            }
        ...
        if(!got){
            if((int)pos.size()>(int)neg.size())cout << "first\n";
            else if((int)pos.size()<(int)neg.size())cout << "second\n";
            else{
                if(last)cout << "first\n";
                else cout << "second\n";
            }
