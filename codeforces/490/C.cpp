...
string s;
int a,b;
int left_number[maxn+5],righ_number[maxn+5];
void solution(){
    cin>>s;
    cin>>a>>b;
    left_number[0]=(s[0]-'0')%a;
    for(int i=1;i<(int)s.size();i++){
        left_number[i]=((left_number[i-1]*10)%a + (s[i]-'0')%a)%a;
    }
    righ_number[(int)s.size()-1] = (s[s.size()-1]-'0')%b;
    int power=10;
    for(int i=(int)s.size()-2;i>=0;i--){
        righ_number[i] = (righ_number[i+1]+ ((s[i]-'0')*power)%b)%b;
        power*=10;
        power%=b;
    }
    int part1=-1,part2=-1;
    for(int i=0;i<(int)s.size()-1;i++){
        if(left_number[i]==0 && righ_number[i+1]==0 && s[i+1]!='0'){
            part1=i;
            part2=i+1;
            break;
        }
    }
    if(part1==-1 or part2==-1)cout << "NO\n";
    else{
        cout << "YES\n";
        for(int i=0;i<=part1;i++)cout <<s[i];
        cout << endl;
        for(int i=part2;i<(int)s.size();i++)cout << s[i];
        cout << endl;
    }
}
signed main()
