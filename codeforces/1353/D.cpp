...
void solution(){
    ...
    while(!pq.empty()){
    	...
    	arr[point]=i++;
    	//debug(l,r,point);
    	int leftl=l,leftr=point-1,szl=leftr-leftl+1;
    	//debug(leftl,leftr,szl);
    	if(szl>=1){
    		pq.push(mp(szl,mp(leftl,leftr)));
    	}
    	int rightl=point+1,rightr=r,szr=rightr-rightl+1;
    	//debug(rightl,rightr,szr);
    	if(szr>=1){
    		pq.push(mp(szr,mp(rightl,rightr)));
    	}
    }
    for(int i=1;i<=n;i++){
    	cout << arr[i] << " ";
