...
int n;
int mat[maxn][maxn];
int md[2*maxn+10],sd[2*maxn+10],ans[2];
pair<int,int>vec[2];
void chk(int oe,int i,int j, int val){
    if(val>ans[oe]){
        ans[oe]=val;
        vec[oe].fi=i+1;
        vec[oe].se=j+1;
...
void solution(){
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            chk((i+j)%2,i,j,md[i+j]+sd[i-j+n]-mat[i][j]);
        }
    }
    cout << ans[0]+ans[1] << endl;
    cout << vec[0].fi << " "<< vec[0].se << " "<< vec[1].fi << " "<< vec[1].se << endl;
}
signed main()
{
    IOS
