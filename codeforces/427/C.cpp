...
ll cost[maxn],n,m,cnt;
vector<ll> adj[maxn],rev[maxn],vis(maxn),vis2(maxn),scc_counter[maxn];
stack<ll> st;
void dfs(int u){
    vis[u]=1;
...
void solution(){
    ...
    for(int i=1;i<=cnt;i++){
        ll mini=highest(int);
        map<ll,ll>mm;
        for(auto it: scc_counter[i]){
            mm[cost[it]]++;
            mini=min(mini,cost[it]);
        }
        ans1+=mini;
        ans2= (ans2%MOD*mm[mini]%MOD)%MOD;
    }
    cout << ans1 << " "<< ans2 << endl;
